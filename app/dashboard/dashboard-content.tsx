...
export function DashboardContent({ pilots, flights, stats }: DashboardContentProps) {
  ...
  return (
    <div className="min-h-screen bg-[#C7D0D7] p-8">
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          >
            <Card className="transition-all duration-300 hover:shadow-lg hover:-translate-y-1 group overflow-hidden">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{item.title}</CardTitle>
                <item.icon className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold group-hover:text-[#0D73B1] transition-colors duration-300">
                  {item.value}
                </div>
              </CardContent>
              <motion.div
                className="absolute inset-x-0 bottom-0 h-1 bg-[#0D73B1]"
                initial={{ scaleX: 0 }}
                whileHover={{ scaleX: 1 }}
